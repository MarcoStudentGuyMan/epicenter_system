...
function createData(unit_id, unit_name, unit_price, unit_status, handleDelete, navigate) {
  return {
    ...
    actions: (
      <>
        <CustomButton className="edit"  color="warning" onClick={() => navigate('/edit_unit_stall_admin')}>
          <IonIcon icon={pencil}  />
          <span>Edit</span>
        </CustomButton>
        <CustomButton className="delete" color = "error" onClick={() => handleDelete(unit_id)}>
